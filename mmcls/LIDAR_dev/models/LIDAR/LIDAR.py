...
from mmcls.LIDAR_dev.models.modules.patch_embed import ConvPatchEmbed
from mmcls.LIDAR_dev.models.LIDAR.LIDAR_layer import LIDARLayer
from models.LDMK_Conv import LDMK_Conv
from models.DPDD import DPDD

@BACKBONES.register_module()
class LIDAR(BaseBackbone):
    arch_zoo = {
        'Crack': {
            'patch_size': 8,
            'embed_dims': 256,
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(LIDAR, self).__init__(init_cfg)
        ...
        self.conv256to256 = LDMK_Conv(256, 256, 32, kernel_sizes=[3, 5, 7])
        self.conv256to64 = LDMK_Conv(256, 64, 32, kernel_sizes=[3, 5, 7])
        self.gn256 = nn.GroupNorm(num_channels=256, num_groups=16)
        self.gn64 = nn.GroupNorm(num_channels=64, num_groups=4)
        self.DPDD = DPDD(channels=256)
    ...
    def forward(self, x, scan_orders):
        ...
        outs = []
        outs_mid_patch_token = []
        for i, layer in enumerate(self.layers):
            x = layer(x, scan_orders, hw_shape=patch_resolution)
            ...
            if i in self.out_indices:
                ...
                patch_token = self.gn256(self.conv256to256(patch_token))
                outs.append(patch_token)

                if i == self.out_indices[0]:
                    patch_token_mid = self.gn64(self.conv256to64(patch_token))
                    outs_mid_patch_token.append(patch_token_mid)
                elif i == self.out_indices[1]:
                    patch_token_mid = self.gn64(self.conv256to64(patch_token)) + outs_mid_patch_token[self.out_indices[0]]
                    outs_mid_patch_token.append(patch_token_mid)
                elif i == self.out_indices[2]:
                    patch_token_mid = self.gn64(self.conv256to64(patch_token)) + outs_mid_patch_token[self.out_indices[1]]
                    outs_mid_patch_token.append(patch_token_mid)
                elif i == self.out_indices[3]:
                    patch_token_mid = self.gn64(self.conv256to64(patch_token)) + outs_mid_patch_token[self.out_indices[2]]
                    outs_mid_patch_token.append(patch_token_mid)
                else:
                    continue

        return outs_mid_patch_token
